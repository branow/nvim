...
function! db#adapter#sqlserver#input(url, in) abort
  let cmd_parts = db#adapter#sqlserver#interactive(a:url) + ['-s', '|', '-i', a:in]
  let shell_cmd = join(map(copy(cmd_parts), 'shellescape(v:val)'), ' ')
  let script = stdpath('config') . '/scripts/sqlcmd-format.awk'
  return ['sh', '-c', shell_cmd . ' | awk -F''|'' -f ' . shellescape(script)]
endfunction
